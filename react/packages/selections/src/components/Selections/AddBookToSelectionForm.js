...
function AddBookToSelectionForm({books, selections, onAddBookToSelection, onError}) {
  ...
  const onSubmit = () => {
    if (bookId && selectionId) {
      onAddBookToSelection({bookId, selectionId})
    } else {
      onError(showError("Please select book and selection"))
    }
  }
